...
function installPrompt(baseUrl, access = {}) {
  ...
  const connection = access.connect
    ...
    : "";
  return `帮我接入 QiaoPage，以后在对话中直接发布网页和文档。

阅读 ${baseUrl}/skill.md，安装或原位更新 qiaomu-quickshare Skill 和 CLI，并保留已有配置。已有可用连接就复用；附有连接码时按 Skill 从标准输入安全兑换；没有连接时直接注册新的个人空间，不需要邀请码。
...
\`\`\`sh
node quickshare.js register --url ${baseUrl}
...
If no code is attached and there is no working connection, use \`register\`; do not ask for an invitation or administrator token. Existing members may copy a fresh Prompt from their signed-in QiaoPage page to connect the same space. The legacy invitation and personal-token \`login --token-stdin\` flows remain supported.
