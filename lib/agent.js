...
function installPrompt(baseUrl, access = {}) {
  ...
  const connection = access.connect
    ...
    : "";
  return `帮我接入 Qiaomu Page，以后在对话中直接发布网页和文档。

阅读 ${baseUrl}/skill.md，安装或原位更新 qiaomu-page Skill 和 CLI，并保留已有配置。已有可用连接就复用；附有连接码时按 Skill 从标准输入安全兑换；没有连接时直接注册新的个人空间，不需要邀请码。

完成 whoami 与 capabilities 检查。新空间只问我用户名，再由 CLI 生成初始密码并保存到本机私密文件；不要在聊天中显示或询问密码。完成后返回管理后台链接、可点击的密码文件链接、修改密码的位置，并给出“发布、更新、列出、下架、恢复”等自然语言示例。明确说明“本次只完成接入，不发布内容”，没有发布任何作品。${invitation}${connection}`;
}

function agentSkill(baseUrl) {
  return `---
name: qiaomu-page
description: "Publish, update, and manage static sites with Qiaomu Page. Use for Qiaomu Page account/site tasks and requests such as 发布到QP, 发布到 qp, or 更新QP."
metadata:
  author: 向阳乔木
  version: "2.1.0"
---
...
GitHub: https://github.com/joeseesun/qiaomu-page
...
Before publishing or updating, run \`node qiaomu-page.js check SOURCE --json\`. Review every finding and fix hard errors. Browser storage findings require either a configured isolated content origin or a guarded fallback for legacy \`/s/\` links. A successful upload and HTTP 200 are not rendering proof: after publication, open the returned URL in a real browser, check page-owned console errors, visible primary content and desktop/mobile layout before calling it complete.

## Product name and triggers

The product is **Qiaomu Page** and its conversation shorthand is **QP**. Match \`qp\` case-insensitively with or without surrounding spaces. Reuse the installed \`qiaomu-page\` Skill, CLI and configured publishing profile. Do not install a duplicate Skill, switch accounts or create a new space because the user uses a legacy name.

Examples:
- “把这个网页发布到 QP” / “发布到 qp” / “发布到 Qiaomu Page” → publish the selected content.
- “用 QP 分享这份 Markdown” / “用 qp 分享这个文件夹” → publish the selected document or static folder.
- “更新 QP 上的这个网页，保持链接不变” → update the identified existing work, preserving its URL.
- “看看我在 QP 上发布了什么” → list works without uploading anything.
- “修改 qp 的用户名” / “修改 Qiaomu Page 的分享设置” → use the corresponding account or sharing tools after reading live identity and permissions.

Resolve “这个 / 刚才的” from the current conversation. Ask only for a missing file/content or update target when it cannot be identified. A bare “qp”, a question about the name, or an unrelated reference such as quadratic programming is not publishing authorization. Legacy Quickshare and QS wording may still identify an existing connection, but do not present them as current product names. These are natural-language triggers, not shell aliases. All ownership, visibility, file-selection and credential rules still apply.

## Install and connect

An installation request authorizes setup only. Do not publish any files until the user chooses what to share.

1. Check for an existing Qiaomu Page or legacy Quickshare CLI and configuration; reuse them when they already point to ${baseUrl}. Do not overwrite another server's configuration.
...
3. Download ${baseUrl}/client/qiaomu-page.js to a suitable user-owned tools directory, preserving existing files. This CLI is a standalone script using Node built-ins; no npm dependencies are needed for normal publishing. Record its absolute path. The commands below use \`qiaomu-page.js\` as shorthand for that path.
4. If the agent supports local skills, save this document as \`SKILL.md\` inside a \`qiaomu-page\` folder in that agent's supported skills directory. Preserve the YAML frontmatter. If the legacy \`qiaomu-quickshare\` Skill exists, migrate it to this name instead of keeping both. Retain user-specific publishing profiles, paths and instructions; never reset a connection during the rename. Follow the agent's documented installation conventions. If skills are unsupported, use the CLI directly and explain the scope.
5. Run \`doctor --json\` if a configuration exists. Reuse a working connection. An attached invitation authorizes the legacy invitation flow; an attached connection code authorizes connecting that existing space. Preserve an already registered account and never reset its password during setup.
6. With no existing connection or attached code, run \`register\` to create a personal space without an invitation. For a legacy invitation, run \`join\`; for a connection code, run \`connect\`. Supply codes through process stdin using a tool's stdin facility or a private mode-600 temporary file. Do not interpolate codes into command arguments, shell history or logs, and do not repeat them in chat. Remove a temporary code file after use. Do not automate webpage form entry.

\`\`\`sh
node qiaomu-page.js register --url ${baseUrl}
# Legacy invitation, when one is explicitly attached:
node qiaomu-page.js join --url ${baseUrl} --invite-stdin < PRIVATE_INVITE_FILE
# OR: connect the same space the user already uses in the browser
node qiaomu-page.js connect --url ${baseUrl} --code-stdin < PRIVATE_CONNECTION_FILE
node qiaomu-page.js doctor --json
\`\`\`
...
If no code is attached and there is no working connection, use \`register\`; do not ask for an invitation or administrator token. Existing members may copy a fresh Prompt from their signed-in Qiaomu Page page to connect the same space. The legacy invitation and personal-token \`login --token-stdin\` flows remain supported.

New installations store configuration at \`~/.config/qiaomu-page/config.json\` with mode 600 and may use \`QIAOMU_PAGE_CONFIG\` for another server. Existing \`~/.config/quickshare/config.json\` and \`QUICKSHARE_CONFIG\` settings remain valid and must be reused rather than copied into a second profile. Do not display configuration contents.
...
4. Give the user a clickable local link to the password file when the client supports local file links; otherwise give its absolute path. Never open, read, quote or paste the password into chat. Tell the user to store it in a password manager and that Qiaomu Page → “账号设置” can replace it later.
...
After installation or registration is verified, make the result usable without requiring the user to know CLI commands. Reply in the user's language and include:

- the connected username and Qiaomu Page instance;
- a clickable management URL at \`${baseUrl}/dashboard\`;
- the clickable local password file when this setup generated one, without reading or quoting its contents;
- where to change it: sign in, open “账号”, then use “账号设置 → 设置新密码”;
- several natural-language examples such as “发布这个网页到 QP”, “更新 QP 上的这个作品，保持链接不变”, “列出我在 QP 上的作品”, “下架这个作品”, and “恢复刚才下架的作品”;
- an explicit statement that setup did not publish any work.

For an existing registered account, omit the password-file item instead of implying a new password was created. Never put a Bearer token, dashboard one-use URL, invitation, connection code or password in chat.
...
node qiaomu-page.js publish ./article.md
node qiaomu-page.js publish ./dist
node qiaomu-page.js update RETURNED_SLUG ./dist
node qiaomu-page.js list --json
node qiaomu-page.js versions my-tool
node qiaomu-page.js rollback my-tool 1
node qiaomu-page.js unpublish my-tool
node qiaomu-page.js restore my-tool
\`\`\`
...
When the instance supports account URLs, the server assigns one stable account handle and a readable project path, such as \`https://alice.example.com/my-tool/\`. It removes generic title words and adds a numeric suffix only for a collision. Use \`--path\` only when the user explicitly requests a different project path; it is fixed after first publication. \`--slug\` remains an advanced compatible internal address. Save the returned slug with the source association so later updates target the same site. Changing the title or account username does not change an existing URL.
...
- Legacy \`/s/\` and all restricted links use an opaque sandbox origin. A configured account content origin may provide localStorage shared by that account's public projects; different account subdomains remain isolated. Management cookies/APIs and Service Workers remain unavailable.
...
function capabilities(baseUrl, state, options = {}) {
  ...
  return {
    ...
    publishing: {
      ...
      contentOrigins: {
        enabled: Boolean(
          options.accountOriginTemplate || options.contentOriginTemplate,
        ),
        mode: options.accountOriginTemplate ? "account-path" : "work-host",
        template:
          options.accountOriginTemplate || options.contentOriginTemplate || null,
        recommendation: options.accountOriginTemplate
          ? "stable-account-handle/readable-project-path"
          : "username-work-random",
