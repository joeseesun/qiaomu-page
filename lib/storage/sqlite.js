"use strict";
const { connectDatabase, ensureColumns } = require("./database");
async function openDatabase(dbPath, provided, env = process.env) {
  const db = provided || connectDatabase(dbPath, env);
  try {
    if (db.raw)
      db.raw.exec(
        "PRAGMA journal_mode=WAL; PRAGMA foreign_keys=ON; PRAGMA busy_timeout=5000",
      );
    await db.transaction(async () => {
      await db.exec(`
    CREATE TABLE IF NOT EXISTS works (
      slug TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT NOT NULL DEFAULT '',
      tags TEXT NOT NULL DEFAULT '[]', theme TEXT NOT NULL DEFAULT 'sage', html TEXT NOT NULL,
      published INTEGER NOT NULL DEFAULT 1, revision INTEGER NOT NULL DEFAULT 1,
      created_at TEXT NOT NULL, updated_at TEXT NOT NULL, cover TEXT, cover_mime TEXT
    );`);
      await ensureColumns(db, "works", {
        access_mode: "TEXT NOT NULL DEFAULT 'public'",
        access_revision: "INTEGER NOT NULL DEFAULT 0",
        share_enabled: "INTEGER NOT NULL DEFAULT 0",
        search_indexable: "INTEGER NOT NULL DEFAULT 0",
        share_title: "TEXT NOT NULL DEFAULT ''",
        share_description: "TEXT NOT NULL DEFAULT ''",
        share_image: "TEXT",
        share_image_mime: "TEXT",
        share_revision: "INTEGER NOT NULL DEFAULT 0",
        owner_id: "INTEGER NOT NULL DEFAULT 1",
        listed: "INTEGER NOT NULL DEFAULT 1",
        files: "TEXT NOT NULL DEFAULT '[]'",
        file_count: "INTEGER NOT NULL DEFAULT 1",
        byte_size: "INTEGER NOT NULL DEFAULT 0",
        content_label: "TEXT NOT NULL DEFAULT ''",
        content_handle: "TEXT NOT NULL DEFAULT ''",
        content_path: "TEXT NOT NULL DEFAULT ''",
      });
      await db.exec("CREATE UNIQUE INDEX IF NOT EXISTS works_content_label ON works(content_label) WHERE content_label<>''");
      await db.exec("CREATE UNIQUE INDEX IF NOT EXISTS works_account_path ON works(content_handle,content_path) WHERE content_handle<>'' AND content_path<>''");
      await db.exec("CREATE TABLE IF NOT EXISTS content_accounts(owner_id INTEGER PRIMARY KEY,handle TEXT NOT NULL UNIQUE)");
      await db.exec(
        "CREATE TABLE IF NOT EXISTS versions(slug TEXT NOT NULL,revision INTEGER NOT NULL,snapshot TEXT NOT NULL,created_at TEXT NOT NULL,PRIMARY KEY(slug,revision))",
      );
      await db.exec(`CREATE TABLE IF NOT EXISTS viewer_sessions (
        hash TEXT PRIMARY KEY, slug TEXT NOT NULL, proof TEXT NOT NULL, access_revision INTEGER NOT NULL, expires INTEGER NOT NULL);`);
      await db.exec(`CREATE TABLE IF NOT EXISTS share_links (
        id TEXT PRIMARY KEY, slug TEXT NOT NULL REFERENCES works(slug), hash TEXT NOT NULL UNIQUE,
        name TEXT NOT NULL, expires INTEGER, revoked INTEGER NOT NULL DEFAULT 0, created_at INTEGER NOT NULL);`);
      await db.exec(`CREATE TABLE IF NOT EXISTS update_requests (
        member_id INTEGER NOT NULL, request_id TEXT NOT NULL, payload_hash TEXT NOT NULL, slug TEXT NOT NULL,
        PRIMARY KEY(member_id,request_id));`);
      await db.exec(`CREATE TABLE IF NOT EXISTS publish_requests (
    member_id INTEGER NOT NULL, request_id TEXT NOT NULL, payload_hash TEXT NOT NULL,
    slug TEXT NOT NULL REFERENCES works(slug), PRIMARY KEY(member_id,request_id)
  )`);
    });
    return db;
  } catch (error) {
    db.close();
    throw error;
  }
}
module.exports = { openDatabase };
