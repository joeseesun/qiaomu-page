...
- 推荐先配置通配 DNS/TLS，再设置 `ACCOUNT_ORIGIN_TEMPLATE=https://{handle}.example.com`，公开地址将是 `https://用户名.example.com/项目名/`。同一账号的公开作品共享该账号 origin 的 localStorage，不同账号由子域隔离。迁移期可同时保留旧 `CONTENT_ORIGIN_TEMPLATE=https://{label}.pages.example.com`；两个内容域都不得与 `BASE_URL` 管理主机相同，也不得携带管理站域 Cookie。
- 默认端口仅绑定宿主机 `127.0.0.1:8090`。可在 `.env.docker` 设置 `QIAOMU_PAGE_PORT=8091`；本地访问时同时修改 `BASE_URL`。不要与已有服务占用同一端口。
