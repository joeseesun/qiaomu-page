...
首次接入时，Agent 会把账号找回能力一起配好：

```text
Agent：你想用什么用户名？
你：qiaomu
Agent：注册完成。[点击查看初始密码]（实际使用时指向你本机的私密文件）
       登录后可在“账号设置”中修改密码。
```
...
![账号设置：修改用户名或替换初始密码](docs/assets/account-settings.png)

初始密码只是为了让账号可登录、可找回。用户查看本地密码文件后登录，即可在“账号设置”换成自己的密码；作品、稳定链接和 Agent 连接不变。
...
不需要记 CLI 命令，可以直接对 Agent 说：「发布这个网页到 QP」「更新 QP 上的这个作品，保持链接不变」「列出我在 QP 上的作品」「下架这个作品」「恢复刚才下架的作品」「设为仅自己可见」「生成一个 7 天有效的分享链接」。
...
**CLI 支持作品的增删改查吗？** 支持创建、列表与单项读取、原链接更新、历史版本和回滚。删除采用可恢复下架：`unpublish` 停止公开访问，`restore` 恢复，不提供容易误操作的永久删除。
