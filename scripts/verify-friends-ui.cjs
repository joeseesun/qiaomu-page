...
(async () => {
  ...
  try {
    ...
    await friend.getByRole("heading", { name: "注册你的发布空间" }).waitFor();
    ...
    await friend.screenshot({path: path.join(root, "artifacts/friends-guest-invite-mobile.png"),fullPage:true});
    await friend.setViewportSize({width:1440,height:1000});
    await friend.locator("#auth-form [name=username]").fill("xiaolin");
