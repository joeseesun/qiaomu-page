...
function indexable(row) {
  return Boolean(
    (!row.access_mode || row.access_mode === "public") &&
      row.published &&
      row.search_indexable &&
      !metadata(row.html).tags.noindex,
  );
}
const cache = new Map();
...
function installSharing({
  ...
  app.get("/social/qiaomu-page.png", productCard);
  app.get("/social/quickshare.png", productCard);
  app.get("/social/:slug", async (req, res) => {
    ...
    const current =
      ...
      (await db
        .prepare(
          "SELECT published,share_enabled,access_mode FROM works WHERE slug=?",
        )
        .get(row.slug));
    if (
      !current?.published ||
