...
两个平台都需要 `QUICKSHARE_TOKEN`。用密码管理器生成至少 32 字符的随机密钥，保存后填入向导对应字段。不要使用示例密码，也不要放入仓库、按钮 URL 或公开聊天。这不是平台 API Token，也不是以后朋友使用的登录密码。
...
3. 输入并私密保存 `QUICKSHARE_TOKEN`，按平台显示的计划和条款确认部署。
4. 部署成功后打开平台给出的生产地址，访问 `/healthz` 应返回 `{"ok":true,"service":"quickshare"}`，再完成下方的管理员接入。
...
4. 输入 `QUICKSHARE_TOKEN`。仓库的 `.dev.vars.example` 仅声明这个必填 secret，不含默认密钥。部署后打开 `workers.dev` 地址，检查 `/healthz`，然后连接管理员。
...
```text
我刚用部署按钮在【Vercel 或 Cloudflare】安装了自己的 QiaoPage，
实例地址是【平台返回的 HTTPS 生产地址】。
请读取这个实例的 /skill.md，安装或复用 qiaomu-quickshare 与独立 CLI。
先检查已有连接。若这是新的实例，请使用独立的私密配置，不覆盖已有账号。
管理员密钥保存在我的密码管理器或该平台的 QUICKSHARE_TOKEN secret 中。
...
```sh
export QUICKSHARE_CONFIG="$HOME/.config/quickshare/my-qiaopage.json"
read -r -s qp_admin_secret
printf '%s' "$qp_admin_secret" | env -u QUICKSHARE_TOKEN -u QUICKSHARE_URL \
  node bin/quickshare.js login --url https://YOUR-INSTANCE --token-stdin
unset qp_admin_secret
env -u QUICKSHARE_TOKEN -u QUICKSHARE_URL node bin/quickshare.js dashboard
```
